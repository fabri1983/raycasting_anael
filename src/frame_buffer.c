...
void loadPlaneDisplacements () {
	for (u16 column = 0; column < PIXEL_COLUMNS; column++) {
		...
		// column is odd => base offset is VERTICAL_COLUMNS*PLANE_COLUMNS which is after the first plane
		frame_buffer_pxcolumn[column] = column&1 ? VERTICAL_COLUMNS*PLANE_COLUMNS + column/2 : 0 + column/2;
	}
}
